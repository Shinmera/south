...
down automatically after a single request."
  ...
  (labels ((ht-symb (name)
             (find-symbol (string-upcase name) "HUNCHENTOOT"))
           (ht-func (name &rest params)
             (apply (symbol-function (ht-symb name)) params))
           (hunchentoot (name)
             (symbol-value (ht-symb name)))
           ((setf hunchentoot) (value name)
             (setf (symbol-value (ht-symb name)) value)))
    (let ((server (make-instance (ht-symb :easy-acceptor) :port *server-port*))
          (standard-output *standard-output*))
      (ht-func :start server)
      (handler-bind
          ((error #'(lambda (err) (declare (ignore err)) (ht-func :stop server))))
        (labels ((dispatcher (request)
